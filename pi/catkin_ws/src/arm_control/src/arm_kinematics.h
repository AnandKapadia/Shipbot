...
public:
	...
	//calculate the FK for 5-DOF team D arm
	geometry_msgs::Pose forwardKinematics(const std::vector<double> &alpha);

	//Calculate IK for arm
	bool inverseKinematics(const geometry_msgs::Pose &pose, std::vector<double> &alpha);
	Eigen::MatrixXd MinimumJerkCoefficients(std::vector<double> q0,
